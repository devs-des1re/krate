...
type PluginHooks struct {
	...
	// Collections returns content collections this plugin contributes to the
	// site. Contributed collections are merged into the effective content
	// config alongside any `content:` declarations in krate.config.ts and are
	// validated, typed, and exposed to the MCP content tools like configured
	// collections. The plugin remains responsible for rendering its own pages.
	Collections func(cfg *config.Config) []CollectionContribution
}

// CollectionContribution is a content collection a plugin contributes. The
// Name is the collection name (e.g. "docs"); Dir is the collection directory
// relative to the project root; Schema optionally carries a validation schema
// for tooling that surfaces collections (MCP, build validation).
type CollectionContribution struct {
	Name   string
	Dir    string
	Schema map[string]content.Field
}
...
// ContributedCollections returns every collection contributed by registered
// plugins, keyed by collection name. Later declarations (higher plugin order)
// override earlier ones per name.
func ContributedCollections(reg *Registry, cfg *config.Config) map[string]content.Collection {
	out := map[string]content.Collection{}
	for _, p := range reg.Sorted() {
		h := p.Hooks()
		if h.Collections == nil {
			continue
		}
		for _, c := range h.Collections(cfg) {
			out[c.Name] = content.Collection{Dir: c.Dir, Schema: c.Schema}
		}
	}
	return out
}

// DefaultContributedCollections returns plugin-contributed collections from the
// default registry.
func DefaultContributedCollections(cfg *config.Config) map[string]content.Collection {
	return ContributedCollections(DefaultRegistry, cfg)
}
